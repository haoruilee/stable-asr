...
The command prints `release_environment_ready`, `final_inputs_ready`,
`final_assignment_ready`, and `final_handoff_ready`. If the release environment
is missing optional backends, the output includes the exact install hint for the
paper-facing path. Final scale remains `NO` until the data paths exist, the
...
handoff has checksums generated by `final-handoff-checksums`, schema evidence
from `validate-schema-file --schema-id stable_asr.final_handoff.v0`, and passes
`final-handoff-audit --require-checksums`. Run `paper-status` after smoke or
release-smoke to get the same gate status plus a `Next Actions` section with
the exact final input, assignment, handoff, and release commands.
...
artifacts, but the release audit will stay `NOT_READY`, print
`release_environment_ready: NO`, and point to the missing NanoTurn or Lance
gates.
