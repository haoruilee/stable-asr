...
- `completion-audit` maps the top-level Stable-ASR objective to roadmap, platform parity, paper bundle, reference evidence, final inputs, assignment, handoff, and final-release gates, then prints next-action commands for unresolved parity, reference, final-input, assignment, handoff, and release blockers
...
- NanoTurn training configs validate against `stable_asr.nanoturn_train_config.v0`
...
- `reference-workqueue --audit-evidence --require-content` can distinguish reference plans from completed evidence by checking every included evidence target and required license-review file on disk, then rejecting empty or placeholder content
...
- unified reference work queue and reference assignment JSON/JSONL/TSV/Markdown files are included in the artifact bundle and pass schema/content audit
