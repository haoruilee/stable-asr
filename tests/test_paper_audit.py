...
def test_paper_audit_accepts_results_and_bundle(tmp_path: Path) -> None:
    ...
    assert "starter_pack:final_acquisition_issue_index" in report.to_text()
    assert "starter_pack:final_acquisition_issue_template" in report.to_text()
    ...
    assert "starter_pack:contributor_reference_evidence_audit" in report.to_text()
    assert "starter_pack:contributor_reference_evidence_templates" in report.to_text()
    assert "starter_pack:contributor_reference_assignments" in report.to_text()
    assert "starter_pack:contributor_reference_evidence_audit_command" in report.to_text()
    assert "starter_pack:contributor_reference_evidence_template_command" in report.to_text()
    assert "starter_pack:contributor_reference_assignment_audit_command" in report.to_text()
    ...
    assert "reference_evidence_templates:markdown" in report.to_text()
...
def test_paper_audit_rejects_tampered_reference_workqueue(tmp_path: Path) -> None:
    result = run_paper_smoke(tmp_path / "paper", episodes=9, seed=6, train_model=False)
    bundle = paper_artifact_bundle(result.results_path, tmp_path / "artifacts")
    workqueue = Path(bundle.reference_workqueue["json"])
    payload = json.loads(workqueue.read_text(encoding="utf-8"))
    payload["tasks"] = [task for task in payload["tasks"] if task.get("collection_type") != "turn"]
    workqueue.write_text(json.dumps(payload), encoding="utf-8")
...
def test_paper_release_audit_reports_remaining_release_gaps(tmp_path: Path) -> None:
    ...
    assert "OK model/model_config_audit" in text
