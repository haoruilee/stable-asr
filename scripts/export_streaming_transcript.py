"""Normalize a precomputed vendor ASR JSONL export for Stable-ASR final runs.

This bridge does not run upstream ASR inference. It validates a shared ASR
manifest, converts a checked vendor raw export to Stable-ASR
StreamingASRRecord JSONL, enriches missing reference/audio metadata from the
manifest, and fails if record IDs do not cover the manifest.
"""
...
from stable_asr.data.converters.streaming_asr import ASR_TRANSCRIPT_SCHEMAS, convert_streaming_asr_rows
from stable_asr.data.formats.jsonl import iter_jsonl, write_jsonl


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--schema", choices=ASR_TRANSCRIPT_SCHEMAS, required=True)
    ...
    records = convert_streaming_asr_rows(raw_rows, schema=args.schema)
    rows = _enrich_records(records, manifest_records, schema=args.schema)
    _validate_coverage(rows, manifest_records)
    write_jsonl(args.output, rows)
    print(f"converted {len(rows)} {args.schema} record(s) to {args.output}")
    return 0
