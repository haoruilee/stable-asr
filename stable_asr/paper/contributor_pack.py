...
def build_contributor_pack(output_dir: str | Path) -> ContributorPackReport:
    ...
    files["reference_license_review_templates_markdown"] = _write_text(
        output_dir / "references" / "REFERENCE_LICENSE_REVIEW_TEMPLATES.md",
        reference_workqueue_license_review_markdown(reference_workqueue),
    )
...
def _starter_commands() -> list[str]:
    return [
        ...
        "stable-asr reference-workqueue --format issues-markdown --output references/REFERENCE_COLLECTION_ISSUES_CURRENT.md",
        "stable-asr reference-workqueue --format license-review-markdown --output references/REFERENCE_LICENSE_REVIEW_TEMPLATES_CURRENT.md",
        "stable-asr reference-workqueue --audit-evidence --require-content --repo-root . --output references/REFERENCE_EVIDENCE_AUDIT_CURRENT.md || true",
